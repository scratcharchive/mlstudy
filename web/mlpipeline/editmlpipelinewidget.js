 ...
 */
function EditMLPipelineWidget(O) {
	O=O||this;
	JSQWidget(O);
	O.div().addClass('EditMLPipelineWidget');

	this.setProcessorManager=function(PM) {m_table_widget.setProcessorManager(PM);};
	...
	var m_table_widget=new EditMLPipelineTableWidget();
	...
	function setPipeline(pipeline) {
		...
		if (pipeline) {
			...
			m_button_widget.setIsPipelineScript(false);
		}
		...
		function do_set_content() {
			if (pipeline) {
				...
				set_to_script_editor_if_different(m_script_editor,'');
				m_table_widget.setPipelineObject(null);
