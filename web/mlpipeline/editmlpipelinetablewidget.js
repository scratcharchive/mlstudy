 ...
 */
function EditMLPipelineTableWidget(O) {
  O=O||this;
  JSQWidget(O);
  ...
  this.pipelineObject=function() {if (!m_pipeline) return null; return m_pipeline.object();};
  ...
  */
  ...
  function create_new_step(callback) {
    if (!m_processor_manager) {
      alert('Processor manger not set in EditMLPipelineTableWidget');
      return;
    }
    var X=new NewStepDialog(0,m_processor_manager,m_job_manager.document());
  ...
  function edit_step(step_index) {
    ...
    var spec=null;
    if (step.step_type=='processor') {
      spec=m_processor_manager.processorSpec(step.processor_name||'');
      if (!spec) {
